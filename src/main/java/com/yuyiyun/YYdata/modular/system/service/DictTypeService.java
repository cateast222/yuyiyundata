...
import com.yuyiyun.YYdata.modular.system.entity.DictType;
import com.yuyiyun.YYdata.modular.system.mapper.DictTypeMapper;
import com.yuyiyun.YYdata.modular.system.model.params.DictTypeParam;
import com.yuyiyun.YYdata.modular.system.model.result.DictTypeResult;
...
public class DictTypeService extends ServiceImpl<DictTypeMapper, DictType> {
	...
	public void add(DictTypeParam param) {
		...
		// 清除缓存
		if (this.save(entity)) {
			CacheUtil.remove(Cache.SYS_DICT, entity.getName());
		}
	}
	...
	public void delete(DictTypeParam param) {
		...
		if (this.removeById(getKey(param))) {
			CacheUtil.remove(Cache.SYS_DICT, param.getName());
		}
	}
	...
	public void update(DictTypeParam param) {
		...
		if (this.updateById(newEntity)) {
			CacheUtil.remove(Cache.SYS_DICT, newEntity.getName());
		}
	}
	...
	public LayuiPageInfo findPageBySpec(DictTypeParam param) {
		...
		if (ToolUtil.isNotEmpty(param.getCondition())) {
			objectQueryWrapper.and(i -> i.eq("code", param.getCondition()).or().like("name", param.getCondition()));
		}
